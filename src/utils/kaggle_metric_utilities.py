...
def treat_as_participant_error(
    ...
    if isinstance(solution, pd.DataFrame):
        solution_is_all_numeric = all(
            [
                pandas.api.types.is_numeric_dtype(x)
                for x in solution.dtypes.values
            ]
        )
...
def safe_call_score(
    metric_function, solution, submission, **metric_func_kwargs
):
    """
    ...
    """
    try:
        score_result = metric_function(
            solution, submission, **metric_func_kwargs
        )
